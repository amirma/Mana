...
void to_mana_filter(const ManaMessage& buff, mana_filter& fltr) {
...
void to_protobuf(const mana_message& msg, ManaMessage& buff) {
    ...
    ManaMessage_notification_t* notification = buff.mutable_notification();
    // interate through message attributes and add them to the protobuf
    for(auto& attr : msg) {
        ManaMessage_notification_t_attribute_t* att =  notification->add_attribute();
        att->set_name(attr.name().begin);
        att->mutable_value()->set_type(ManaMessage_tag_type_t(attr.type()));
        switch(attr.type()) {
...
void to_protobuf(const mana_filter& fltr, ManaMessage& buff) {
    // set message type
    buff.set_type(ManaMessage_message_type_t_SUB);
    ManaMessage_subscription_t* s = buff.mutable_subscription();
    for(auto& cnst : fltr) {
    	ManaMessage_subscription_t_constraint_t* c = s->add_constraints();
    	c->set_name(cnst.name().begin);
    	c->mutable_value()->set_type(ManaMessage_tag_type_t(cnst.type()));
    	c->set_op(ManaMessage_subscription_t_operator_t(cnst.op()));
        switch(cnst.type()) {
